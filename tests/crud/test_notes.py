...
def test_update_note_db(mock_db, note_sample):
    new_name = "UPDATED " + note_sample.name
    note = update_note_db(
        mock_db,
        note_sample,
        {"name": new_name},
    ...
    assert note.name == new_name
    mock_db.commit.assert_called_once()
    mock_db.refresh.assert_called_once_with(note_sample)